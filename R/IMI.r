...
#'Intraday Relative Momentum Index
#'
#'@aliases IMI
#'@param OC Object that is coercible to xts or matrix and contains
...
#'@export
"IMI" <-
function (OC, n = 14, maType, ...) {
  OC <- try.xts(OC, error = as.matrix)
  if (!is.OHLC(OC)) stop("No Open or Close Data")
  up <- Cl(OC)-Op(OC)
  which.dn <- which(up < 0)
  dn <- up * 0
